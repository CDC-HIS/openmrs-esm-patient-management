...
import { useCalendarDistribution } from '../appointment-forms/workload-helper';
import { PatientAppointment, useInitialAppointmentFormValue } from '../appointment-forms/useInitialFormValues';
...
const AppointmentWidget: React.FC<AppointmentWidgetProps> = ({
  ...
  useEffect(() => {
    if (setHasSubmissibleValue) {
      setHasSubmissibleValue(!patientAppointment.serviceUuid);
    }
  }, [patientAppointment, setHasSubmissibleValue]);
  ...
  return (
    <div className={styles.formContainer}>
      ...
      <Select
        ...
        onChange={(event) => {
          setPatientAppointment({ ...patientAppointment, locationUuid: event.target.value });
          setHasSubmissibleValue(!patientAppointment.serviceUuid);
        }}>
        {locations?.length > 0 &&
